...
namespace Project_for_App_Domain.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private PasswordHistory phistory = new PasswordHistory();
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            // To enable password failures to trigger account lockout, change to shouldLockout: true <- enabled
            var result = await SignInManager.PasswordSignInAsync(model.Username, Hasher.HashString(model.Password), model.RememberMe, shouldLockout: true);
            switch (result)
            {
        ...
        public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                SWE4713Entities e = new SWE4713Entities();
                Models.User userdb = (from s in e.Users
                ...
                e.PasswordHistories.Add(phistory);
                e.SaveChanges();

                var result2 = e.Users.SingleOrDefault(b => b.Email == model.Email);
                if(result2 != null)
                {
                    result2.Attempts = 0;
                    result2.DateUpdated = DateTime.Now;
                    e.SaveChanges();
